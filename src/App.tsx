        ...
        {isMobileMenuOpen && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
            ...
            <div className="px-6 py-12 flex flex-col items-center gap-8">
              <Logo className="h-12" />
              {['Rituale', 'Behandlungen', 'Oase', 'Journal'].map((item) => (
                <a key={item} href={`#${item.toLowerCase()}`} className="text-lg font-serif text-spa-stone"
                  onClick={() => setIsMobileMenuOpen(false)}>{item}</a>
              ))}
        ...
        <div className="flex flex-col items-center gap-6">
          ...
          <div className="flex gap-8">
            {['Datenschutz', 'AGB', 'Karriere', 'Kontakt'].map(item => (
              <a key={item} href="#" className="text-[10px] uppercase tracking-widest text-spa-stone/40 hover:text-spa-nude transition-colors">{item}</a>
            ))}
          </div>
        </div>
      </footer>
